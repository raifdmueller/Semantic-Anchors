...
function parseAnchorFile(filePath) {
  ...
  const parseList = (attr) => {
    if (!attr) return [];
    return attr.split(',').map(item => item.trim()).filter(item => item.length > 0);
  };
...
function generateMetadata(anchors, categories, roles) {
  return {
    ...
    statistics: {
      averageRolesPerAnchor: (anchors.reduce((sum, a) => sum + a.roles.length, 0) / anchors.length).toFixed(2),
      averageCategoriesPerAnchor: (anchors.reduce((sum, a) => sum + a.categories.length, 0) / anchors.length).toFixed(2),
...
function main() {
  ...
  fs.writeFileSync(
    path.join(OUTPUT_DIR, 'anchors.json'),
  ...
  fs.writeFileSync(
    path.join(OUTPUT_DIR, 'roles.json'),
